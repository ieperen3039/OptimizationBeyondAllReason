...
impl DeterministicReinforcementPolicy {
    pub fn new(learner: ReinforcementLearning, random_seed: u32) -> DeterministicReinforcementPolicy {
        let device = learner.get_device();
        let rng = MyRandom::new_from_u32(random_seed);
        let mut learner = ReinforcementLearning::new(1000, rng.next_u32(), 3000.0, Box::from(ResourceGenerationReward));
        let model = learner.train();
        DeterministicReinforcementPolicy { model, device }
    }

    pub fn from_model(model: rl::Model) -> DeterministicReinforcementPolicy {
        Self {
            model,
            device: Cpu::default(),
        }
